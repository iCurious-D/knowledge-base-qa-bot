...
# 【核心改造1：移除原生OpenAI，导入LangChain工具类和多模态消息模块】 LLM客户端工具类（核心复用，替换原生OpenAI调用）
from app.clients.minio_client import get_minio_client
from app.clients.lm_client import get_llm_client
...
def find_image_in_md(md_content: str, img_file: str, context_len: int = 100) -> List[Tuple[str, str]]:
    """ 查找MD内容中指定图片的所有引用位置，并返回每个位置的上下文文本
    :param md_content: MD文件完整内容
    :param img_file: 图片文件名（含后缀）
    ...
    for m in pattern.finditer(md_content):
        ...
        # 打印图片上下文，便于调试
        logger.debug(f"图片[{img_file}]匹配到引用，上文：{pre_text.strip()}")
        logger.debug(f"图片[{img_file}]匹配到引用，下文：{post_text.strip()}")
        results.append((pre_text, post_text))

    if not results:
        logger.debug(f"MD内容中未找到图片[{img_file}]的引用")
...
def step2_scan_images(md_content: str, images_dir: Path) -> List[Tuple[str, str, Tuple[str, str]]]:
    ...
    for img_file in os.listdir(images_dir):
        ...
        if not is_supported_image(img_file):
            logger.debug(f"图片文件格式不支持，跳过：{img_file}")
            continue
        ...
        # 查找图片在MD中的引用上下文
        context_list = find_image_in_md(md_content, img_file)

        # 过滤MD中未引用的图片
        if not context_list:
            logger.warning(f"图片未在MD中引用，跳过处理：{img_file}")
            continue

        # 组装待处理图片元数据，取第一个匹配的上下文
        targets.append((img_file, img_path, context_list[0]))
        logger.info(f"图片加入待处理列表：{img_file}")
...
def encode_image_to_base64(img_path: str) -> str:
    ...
    :param img_path: 图片本地完整路径
    :return: 图片的Base64编码字符串（UTF-8解码）
    """
    with open(img_path, "rb") as f:
        base64_str = base64.b64encode(f.read()).decode("utf-8")
    logger.debug(f"图片Base64编码完成，文件：{img_path}，编码后长度：{len(base64_str)}")
    return base64_str


def summarize_image(img_path: str, root_folder: str, image_content: Tuple[str, str]) -> str:
    ...
    """
    # 将图片编码为Base64，适配多模态模型输入要求
    img_base64 = encode_image_to_base64(img_path)

    try:
        # 1. 获取项目统一LLM客户端（自动缓存，传入多模态模型名）
        vlm_client = get_llm_client(model=lm_config.vlm_model)
        ...
        logger.info(f"图片摘要生成成功：{img_path}，摘要：{summary}")
        return summary

    except LangChainException as e:
        logger.error(f"图片摘要生成失败（LangChain框架异常）：{img_path}，错误信息：{str(e)}")
        return "图片描述"
    except Exception as e:
        logger.error(f"图片摘要生成失败（系统异常）：{img_path}，错误信息：{str(e)}")
        return "图片描述"


def step3_generate_summaries(doc_stem: str, targets: List[Tuple[str, str, Tuple[str, str]]],
    ...
    for img_file, img_path, context in targets:
        # prompt = f"{doc_stem}\n\n图片：{img_file}\n\n图片内容：{context[0]}{context[1]}"
        # summary = get_llm_client().generate(prompt)
        apply_api_rate_limit(request_times, requests_per_minute)
        logger.debug(f"开始生成图片摘要：{img_file}")
        summaries[img_file] = summarize_image(img_path, root_folder=doc_stem, image_content=context)

    logger.info(f"图片摘要批量生成完成，共处理{len(summaries)}张图片")
    return summaries


# 步骤4：图片上传MinIO + 合并摘要&URL + 替换MD图片引用 ===================================================================
def clean_minio_dir(minio_client: Minio, prefix: str) -> None:
    """ 幂等性清理MinIO指定目录下的所有旧文件，防止重名文件内容混淆和垃圾文件堆积; 幂等性：多次调用结果一致，无文件时不报错
...
def upload_images_batch(minio_client: Minio, upload_dir: str,
...
def merge_summaries_and_urls(summaries: Dict[str, str], urls: Dict[str, str]) -> Dict[str, Tuple[str, str]]:
    ...
    image_info = {}
    # 遍历摘要字典，仅保留有对应URL的图片
    for img_file, summary in summaries.items():
        if url := urls.get(img_file):
...
def process_md_file(md_content: str, image_info: Dict[str, Tuple[str, str]]) -> str:
    """ 核心功能：替换MD内容中的本地图片引用为MinIO远程引用;  替换规则：![原描述](本地路径) → ![图片摘要](MinIO访问URL)
...
def step4_upload_and_replace(minio_client: Minio, doc_stem: str, targets: List[Tuple[str, str, Tuple[str, str]]],
                              summaries: Dict[str, str], md_content: str) -> str:
    """ 步骤4：完整流程：清理MinIO旧目录 → 批量上传新图片 → 合并摘要和URL → 替换MD内容
    :param minio_client: 初始化完成的MinIO客户端对象
    ...
    # 步骤2：批量上传图片至MinIO，获取URL
    img_urls = upload_images_batch(minio_client, upload_dir, targets)

    # 步骤3：合并图片摘要和URL，过滤上传失败的图片
    image_info = merge_summaries_and_urls(summaries, img_urls)
...
def node_md_img(state: ImportGraphState) -> ImportGraphState:
    ...
    3. 调用多模态大模型为图片生成内容摘要
    4. 将图片上传至MinIO，替换MD中本地图片路径为 MinIO访问URL，并填充图片摘要
    ...
    # 步骤1：初始化数据，获取MD核心信息: MD内容、文件路径、图片文件夹路径
    ...
    # 步骤2：扫描图片文件夹，筛选MD中实际引用的支持格式图片
    # targets：[（图片文件名, 图片完整路径, 图片上下文）......]
    targets = step2_scan_images(md_content, images_dir)
    if not targets:
        logger.info("未检测到MD中引用的支持格式图片，跳过后续处理")
        return state

    # 步骤3：调用多模态大模型生成图片摘要（修复原代码传参错误：使用文件主名而非MD内容）
    summaries = step3_generate_summaries(path_obj.stem, targets)

    # 步骤4：上传图片至MinIO，替换MD图片路径并填充摘要
    new_md_content = step4_upload_and_replace(minio_client, path_obj.stem, targets, summaries, md_content)
